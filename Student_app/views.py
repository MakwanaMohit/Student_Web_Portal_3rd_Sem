            ...
            if user is not None:
                if user.role == User.Role.STUDENT:
                    ...
                    messages.error(request,
                                   "You logged in wrong Page with these id and password please login in this page")
...
def result(request):
    ...
    session = []
    authenticated = request.user.is_authenticated and request.user.role == User.Role.STUDENT
    if authenticated:
        Form = Student_result
    ...
    if request.method == 'POST':
        ...
        if e_type:
            valid = form.is_valid() or (
                        request.session.get('session-key') == request.POST.get('session-key') and request.POST.get(
